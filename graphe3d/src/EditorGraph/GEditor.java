...
public class GEditor extends JPanel{
	...
	public static void main(String[] args) {
		...
		load_ascii = new JMenuItem("Charger des types..."),
		...
		GGraphUniverse universe = new GGraphUniverse(new GGraph());
		PanelButtonInteraction interaction = new PanelButtonInteraction(universe);
		String filename = "";
		GEditor editor = new GEditor(filename,universe);
...
//		gbl.addLayoutComponent(view, VIEW_CONSTRAINTS);
		 ...
		 */
		editor.listener = new EditorListener();
		editor.listener.owner = frame;
		editor.listener.editor = editor;
		editor.listener.load_ascii = load_ascii;
		editor.listener.load_graph = load_graph;
		editor.listener.save = save;
		editor.listener.save_as = save_as;
		editor.listener.new_graph = new_graph;
		editor.listener.exit = exit;
		/*
		 * adding this listener to every menu items
		 */
		new_graph.addActionListener(editor.listener);
		load_ascii.addActionListener(editor.listener);
		load_graph.addActionListener(editor.listener);
		save.addActionListener(editor.listener);
		save_as.addActionListener(editor.listener);
		exit.addActionListener(editor.listener);

		frame.setSize(800,800);
		frame.setVisible(true);

		frame.addWindowListener(editor.listener);
	}
	 ...
	 */
	public GEditor(String _ascii_file, GGraphUniverse _Universe) {
		/*
		 * editor naming
		 */
	...
	/**
	 * replaces the old GGraphUniverse by a new GGraphUniverse.
	...
	/**
	 * returns the GGraphUniverse which is associated to the GEditor.
	 * @return
	 * 		the associated GGraphUniverse
	 */
	public GGraphUniverse getUniverse(){
		return universe;
	}
				...
				if(button == tabArea.remove){
					...
					if(component instanceof GNode){
						...
						try {
							...
							//normalement ne doit jamais apparaître
							e.showError();
						}
						universe.deleteGLink(((GLink)component).getName());
						tabArea.attributes_list.remove(component);
						tabArea.attributes_list.checkAttachedComponents();
					}
				}else if(button == tabArea.remove_all){
					LinkedList<Object> elements = new LinkedList<Object>(tabArea.attributes_list.getElements());
					for(int i = 0; i < elements.size();i++) {
						if(elements.get(i) instanceof GNode) {
							removeGNode((GNode) elements.get(i));
						} else {
							GLink link = ((GLink)elements.get(i));
							if (universe.getGraph().getLink(link.getName()) != null) {
								try {
									link.getFirstNode().removeLink(link);
									link.getSecondNode().removeLink(link);
								} catch (GException e) {
							...
							tabArea.attributes_list.remove(link);
							tabArea.attributes_list.checkAttachedComponents();
						}
					}
					...
					if(tabArea.attributes_list.getNodeCount()==2){
						LinkedList<Association> list = new LinkedList<Association>();
						GNode
						node1 = (GNode) tabArea.attributes_list.getElements().get(0),
						node2 = (GNode) tabArea.attributes_list.getElements().get(1);
						list.add(new Association(node1, node2));
						GPopup.createLink(GEditor.this,type_of_links, GPopup.ARROW, list);
					}else{
					...
					if(tabArea.attributes_list.getNodeCount()==1){
						...
						GPopup.createLink(GEditor.this,type_of_links, GPopup.BRIDGE, list);
					}else{
						GNode[]nodes = new GNode[tabArea.attributes_list.getNodeCount()];
						for(int i=0; i<nodes.length;i++)
							nodes[i] = (GNode) tabArea.attributes_list.getElements().get(i);
	...
	void removeGNode(GNode node){
		...
		if(list_links.size()>0){
			if(new DeleteNodeWithLinksException(node).showError()){
				...
				tabArea.attributes_list.remove(node);
				tabArea.attributes_list.checkAttachedComponents();
			}//if
		}else{
			universe.deleteGNode(node.getName());
	...
	static class EditorListener extends WindowAdapter implements ActionListener{
		...
		public void actionPerformed(ActionEvent a){
			...
			if(item == load_ascii){
				JFileChooser chooser = new JFileChooser(FILE_BASE);
				int returnVal = chooser.showDialog(this.owner, "chargement");
				if(returnVal == JFileChooser.APPROVE_OPTION){
					FILE_BASE = chooser.getSelectedFile().getParent();
				...
				case 1:
					...
					if(returnVal == JFileChooser.APPROVE_OPTION){
						...
						editor.tabArea.attributes_list.removeAll();
						try{
							String path = FILE_BASE+System.getProperty("file.separator")+FILE_NAME;
		...
		void saveAs(){
			...
			if(returnVal == JFileChooser.APPROVE_OPTION){
				this.FILE_BASE = chooser.getSelectedFile().getParent();
				this.FILE_NAME = chooser.getSelectedFile().getName();
				/*
				 * if there is no ".xml" suffix we set it
				 */
				if(!FILE_NAME.endsWith(".xml")) FILE_NAME+=".xml";
				try{
					String path = FILE_BASE+File.separator+FILE_NAME;
					GParser parser = new GParser(editor.getUniverse().getGraph(),path);
					parser.saveGraph();
				}catch (GException e) {
					e.printStackTrace();
		...
		void exit(){
			boolean exit = true;
			if (!editor.getUniverse().getGraph().getNodes().isEmpty()) {
				Object[] options = new Object[] {"YES", "NO", "CANCEL"};
