...
public class GTab extends JScrollPane{
	...
	throws GException{
		...
		if( _element instanceof GNode){
			...
			int size = node.getAttributes().size()+7;/*type + name + radius + 3 coordonates + exit cross*/
			panel.setLayout(
			...
			panel.setLayout(
					new GridLayout(link.getAttributes().size()+3/*type + name + exit cross*/,1)
			);
			GridLayout layout = new GridLayout(1,2);
			JPanel entry = new JPanel(layout);
			JLabel label = new JLabel("nom  ",JLabel.RIGHT);
				...
				if(attr_name.equals("name")){
					...
					if(!text.getText().equals(old_value)){
						...
						if(lk == null){ // not already busy
							...
							int index = attributes_list.indexOfComponent(GTab.this);
