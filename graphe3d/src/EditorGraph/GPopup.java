...
public class GPopup extends JDialog {
	 ...
	 */
	public static void showPopup(GEditor editor, GNode[]nodes, boolean oriented, String link_type){
		if(POPUP==null)POPUP = new GPopup();
		GPopup.link_type = link_type;
		GPopup.editor = editor;
		list = new LinkedList<Association>();
		GPopup.oriented = oriented;
		if(oriented) // ARROWS
		...
		public void mouseClicked(MouseEvent m){
			if(m.getButton() == MouseEvent.BUTTON1){
				...
				if(button == POPUP.cancel_button)
					...
					createLink(GPopup.editor, link_type, oriented, list);
					POPUP.setVisible(false);
				}//if
			}//if
